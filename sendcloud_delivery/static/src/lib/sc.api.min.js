...
sendcloud.servicePoints = (function () {
    ...
    function _callable(fn) {
        return !fn || typeof fn !== 'function' ? _noop : fn;
    }
    ...
    function validateLanguage(language) {
        ...
        for (var k in updateBrowserMessage) {
            allowedLanguages.push(k);
            var short = k.replace(/-\w+/, '');
            if (allowedLanguages.indexOf(short) === -1) {
                allowedLanguages.push(short);
            }
        }
    ...
    function validateRequired(value) {
        return (value.length > 0 && typeof value === 'string') || value instanceof String;
    }

    function validateConfig(config) {
        var errors = [];
        if (!validateRequired(config.apiKey)) {
            errors.push('Missing API key.');
        }
        if (!validateLanguage(config.language)) {
            errors.push('Invalid language set: ' + config.language);
        }
        if (!validateRequired(config.country)) {
            errors.push('No country set.');
        }
        if (errors.length > 0) {
    ...
    function open(config, onSuccess, onFailure) {
        successCallback = _callable(onSuccess);
        failureCallback = _callable(onFailure);
        ...
        if (msie && msie <= 9) {
            var msg = updateBrowserMessage[language];
            failureCallback([msg]);
            return;
        }
        var params = {
            ...
            id: config.servicePointId || '',
            weight: config.weight || '',
            language: language,
            'close-button': 1,
            'post-number': config.postNumber || ''
        };
        var url = baseDomain + '/embed/v3/service-point-picker/?' + serializeParams(params);
        iframeDiv = document.createElement('div');
        iframeDiv.innerHTML = '<iframe src="' + url + '" width="100%" height="100%" id="sendcloudshipping_service_point_map">';
    ...
    function onServicePointSelected(data) {
        successCallback(data.point, data.postNumber || '');
        removeServicePointPicker();
    }

    function onServicePointClose() {
        failureCallback(['Closed']);
        removeServicePointPicker();
    }
    ...
    function onWindowMessage(event) {
        ...
        var messageHandlers = {servicePointSelected: onServicePointSelected, servicePointClose: onServicePointClose};
        if (!(messageData.type in messageHandlers)) {
            failureCallback(['Invalid event type']);
            return;
        }
    ...
    return {open: open, close: removeServicePointPicker};
})();/* */
