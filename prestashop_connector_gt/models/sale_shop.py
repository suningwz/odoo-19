	...
	def import_customers(self):
		for shop in self:
			...
			try:
				prestashop = PrestaShopWebServiceDict(shop.shop_physical_url,shop.prestashop_instance_id.webservice_key or None)
				# prestashop = PrestaShopWebServiceDict(shop.prestashop_instance_id.location,shop.prestashop_instance_id.webservice_key or None)
				filters = {'display': 'full', 'filter[id]': '>[%s]' % self.last_customer_id_import,'filter[id_shop]': '%s' % self.presta_id, 'limit': 2000}
	...
	def create_presta_product(self, product_dict, prestashop):
		...
		try:
			...
			prd_tmp_vals = {
				...
				'website_description':  self.action_check_isinstance(product_dict.get('description').get('language'))[0].get('value'),
			...
			if product_dict.get('id_tax_rules_group') not in ['0','']:
				tax_group_id = account_tax_group_obj.search([('presta_id','=',product_dict.get('id_tax_rules_group'))],limit=1)
			...
			product_domain=[('prestashop_product','=',True)]
			...
			for child in order_rows:
				...
				if self.get_value_data(child.get('product_attribute_id')) != '0':
					...
					try:
						...
						temp_id = prod_templ_obj.search([('presta_id', '=',self.get_value_data(child.get('product_id'))),	 ('prestashop_product', '=', True)], limit=1)
					except Exception as e:
					...
					if temp_id:
						product_ids = product_obj.search([('presta_id', '=',self.get_value_data(child.get('product_id')))])
						for product_id in product_ids:
							if product_id.product_template_attribute_value_ids == prod_attr_val_obj.browse(value_list) and product_id.product_tmpl_id == temp_id:
