...
class ImportWoocommerceOrders(models.TransientModel):
    ...
    def get_woocommerce_discount_lines(self,woocommerce,data):
        amount = 0
        for line in data["coupon_lines"]:
        ...
        vals = {
            ...
            "line_taxes" : self.env["import.woocommerce.taxes"].create({
                    ...
                    "operation":"import"
                    }).get_woocommerce_taxes(woocommerce,tax_ids)
        }
        return vals
    ...
    def get_woocommerce_order_line(self, woocommerce, channel, data):
        ...
        for line in line_items:
            ...
            order_line_dict = {
                ...
                'line_taxes': self.env["import.woocommerce.taxes"].create({
                    "channel_id":channel.id,
                    "operation":"import"
                    }).get_woocommerce_taxes(woocommerce,line.get("taxes"))
            }
        ...
        if data['shipping_lines']:
            order_lines += self._get_woocommerce_shipping(woocommerce,
                data['shipping_lines'])
        if data["coupon_lines"]:
            discount_line = self.get_woocommerce_discount_lines(woocommerce,data)
    ...
    def _filter_order_using_date(self, woocommerce, channel, **kwargs):
        ...
        if orders:
            ...
            if kwargs.get("from_cron"):
                ...
                channel.import_order_date = fields.Datetime.to_datetime(" ".join(date_created.split("T")))
        return vals_list
        ...
        for shipping in shipping_line:
            shipping_dict = {
                ...
                'line_taxes':self.env["import.woocommerce.taxes"].create({
                    "channel_id":self.channel_id.id,
                    "operation":"import"
                    }).get_woocommerce_taxes(woocommerce,shipping.get("taxes"))
            }
            shipping_list.append((0, 0, shipping_dict))
